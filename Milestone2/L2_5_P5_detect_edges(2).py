    ...
    for y in range(get_height(image) - 1): # begins at the first row and moves down through each row in the image       
        for x in range(get_width(image)):
            ...
            if abs(((r + g + b) // 3) - ((r1 + g1 + b1)// 3)) > threshold:
                set_color(new_image, x, y, create_color(0, 0, 0))  #changes the pixels to black if contrast between the two pixels is high
            else:
...
if __name__ == '__main__':
    detect_edges(load_image(choose_file()), 12) # by changing the threshold value, the ratio of white to black pixels changes
