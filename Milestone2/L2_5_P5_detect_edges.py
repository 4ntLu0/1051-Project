"""ECOR 1051 Fall 2019 Project. L2-5. Milestone 2, P6. Edge Detection. This code has been attributed \
...
def detect_edges(image: Image, threshold: int) -> Image:
    """ Using a selected image, this function alters the r, g, b components of
    an image to return a new image
    that looks like a pencil sketch (aka edge detected image). The pixels in the
    image are changed to either black\
    ...
    for y in range(get_height(new_image) - 1):                              
        ...
        for x in range(get_width(new_image)): 
            ...
            if abs(((r + g + b) // 3) - ((r1 + g1 + b1) // 3)) > threshold: 
                # Changes the pixels to black if contrast between pixels is high.
                set_color(new_image, x, y, create_color(0, 0, 0))
            else:                                                           
                # Changes the pixels to white is contrast between pixels is low
