...
#Imports the Improved Edge Detection function so it can later be called. 

def detect_edges_better_test() -> None:
    """Test function that checks if the r, g, b components have been correctly
    altered by the Improved Edge Detection filter. 
    If the pixels are correctly changed to either white or black, the test 
    function returns "PASS" to the user. If this is not the case, 
    ...
    original_image = create_image(7, 3, color = create_color(10, 10, 10))      
    # Creates image that is width 6 and height 2 set to a dark colour. 
    ...
    for x, y, (r, g, b) in actual_image:
        if (r, g, b) == tuple(get_color(expected_image, x, y)):                 
            # Checks if the pixels equal the ones in the expected image.
            print('PASS')
        else:
            print('FAIL')
